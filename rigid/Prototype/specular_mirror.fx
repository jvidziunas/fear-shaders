...
SAMPLER_WRAP_sRGB(sReflectionMapMaskSampler, tReflectionMapMask);
...
{
	return LinearizeAlpha( tex2D(sReflectionMapMaskSampler, vCoord) );
}
...
struct PSData_Point 
{
	...
	float4 ScreenCoord	: TEXCOORD3_centroid;
};
...
{
	float4 Position			: POSITION;
	float4 TexCoordClip		: TEXCOORD0_centroid;
	float4 ScreenCoord		: TEXCOORD1_centroid;
...
{
	float3 surfaceNormal = GetSurfaceNormal(IN.TexCoordClip.xy);
	float4 specularValue = GetMaterialSpecular(IN.TexCoordClip.xy);
	specularValue.w *= GetToksvigGlossScale(surfaceNormal, specularValue.w * fMaxSpecularPower);
...
struct PSData_CubeProjector
{
	...
	float3 LightVector	: TEXCOORD2_centroid;
	float3 EyeVector		: TEXCOORD3_centroid;
	float3 LightMapCoord	: TEXCOORD4_centroid;
};	
