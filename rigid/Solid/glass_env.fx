...
{
	float4 Position				: POSITION;
	float4 TexCoordAndClipPlanes	: TEXCOORD0;
	float3 LightVector			: TEXCOORD1;
	float3 EyeVector				: TEXCOORD2;
...
float4 SpotProjector_PS(PSData_SpotProjector IN) : COLOR
{
	...
	float4 vPixelColor = GetLitPixelColor(IN.LightVector, IN.EyeVector, 
		GetSurfaceNormal_Unit(IN.TexCoordAndClipPlanes.xy), GetMaterialDiffuse(IN.TexCoordAndClipPlanes.xy, IN.ObjEyeVector, GetSurfaceNormal_Unit(IN.TexCoordAndClipPlanes.xy), IN.TanSpace0, IN.TanSpace1, IN.TanSpace2), GetMaterialSpecular(IN.TexCoordAndClipPlanes.xy), 
