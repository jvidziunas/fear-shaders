...
DECLARE_DESCRIPTION("This is an additive version of the refract shader.  This will fall back to the additive material on DX8 hardware.");
DECLARE_DOCUMENATION("Shaders\\Docs\\refract_additive\\main.htm");
...
float4 Translucent_PS(PSData_Translucent IN) : COLOR
{
	...
	vResult.x = tex2D( sCurFrameMapSampler, uv + vOffset * 0.9 ).x;
	vResult.y = tex2D( sCurFrameMapSampler, uv + vOffset ).y;
	vResult.z = tex2D( sCurFrameMapSampler, uv + vOffset * 1.1 ).z;
	...
	vResult.xyz += vDiffuseMap.xyz * vDiffuseMap.w * fVertAlpha;
